...
int main(int argc, char **argv)
{
    ...
    // 障碍物
    Eigen::MatrixXd obstacle(3, 5);
    obstacle.col(0) << 5.0, 5.0, 2.0;
    obstacle.col(1) << -5.0, -5.0, 2.0;
    obstacle.col(2) << 5.0, -5.0, 2.0;
    obstacle.col(3) << -5.0, 5.0, 2.0;
    obstacle.col(4) << 0.0, 0.0, 4.0;
    ...
    while(ros::ok())
    {
        ...
        if(piece_num != 0)
        {
            ...
            {
                // ROS_INFO_STREAM("The initial way points is \n" << wayPoint);
                // ROS_INFO_STREAM("The way points is \n" << optimal_points);
                // ROS_INFO_STREAM("The real time vector is \n" << optimal_T);
                // minco_ptr_->generate(optimal_points, optimal_T);

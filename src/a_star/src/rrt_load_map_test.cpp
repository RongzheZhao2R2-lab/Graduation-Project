...
void goalCallback(const geometry_msgs::PoseStamped::ConstPtr &goal_ptr)
{
    ...
    if(ret)
    {
        ROS_INFO("\033[32mSearch Success!\033[0m");
        ROS_INFO("\033[32mThe path length : %lf!\033[0m", rrt_ptr->getPathlength());
    }
    else
    {
        ROS_INFO("\033[31mSearch Failed!\033[0m");
        return;
    }   
}
...
{
    ros::init(argc, argv, "rrt_load_gen_test");
    ros::NodeHandle nh("~");
